...
Once the propensity score $\hat{e}(X)$ is estimated (see [[Propensity Score Matching - Balancing Theorem and Failure Modes]]), the matching step constructs a control group that is comparable to the treated group in its propensity-score distribution. Different algorithms make different trade-offs between:
