...
$\left(\right. E \left[\right. Y \left(\right. 1 \left.\right) \left|\right. D = 1 \left]\right. - E \left[\right. Y \left(\right. 1 \left.\right) \left|\right. D = 0 \left]\right. \left.\right) - \left(\right. E \left[\right. Y \left(\right. 0 \left.\right) \left|\right. D = 1 \left]\right. - E \left[\right. Y \left(\right. 0 \left.\right) \left|\right. D = 0 \left]\right. \left.\right) = \left(\right. 87.06 - 206.16 \left.\right) - \left(\right. 46.01 - 171.64 \left.\right) = 6.53$
...
|  | state | year | cigsale | retprice | california | after\_treatment |
| --- | --- | --- | --- | --- | --- | --- |
...
![_images/15-Synthetic-Control_5_0.png](https://matheusfacure.github.io/python-causality-handbook/_images/15-Synthetic-Control_5_0.png)
...
To make matters a little bit more formal, suppose that we have $J + 1$ units. Without loss of generality, assume that unit 1 is the unit that gets affected by an intervention. Units $j = 2 , . . . , J + 1$ are a collection of untreated units that we will refer to as the “donor pool”. Also assume that the data we have span T time periods, with $T_{0}$ periods before the intervention. For each unit j and each time t, we observe the outcome $Y_{j t}$. For each unit j and period t, define $Y_{j t}^{N}$ as the potential outcome without intervention and $Y_{j t}^{I}$, the potential outcome with intervention. Then, the effect for the treated unit $j = 1$ at time t, for $t > T_{0}$ is defined as

$\tau_{1 t} = Y_{1 t}^{I} - Y_{1 t}^{N}$

Since unit $j = 1$ is the treated one, $Y_{1 t}^{I}$ is factual but $Y_{1 t}^{N}$ is not. The challenge then becomes how do we estimate $Y_{1 t}^{N}$. Notice how the treatment effect is defined for each period, which means it can change in time. It doesn’t need to be instantaneous. It can accumulate or dissipate. To put it in a picture, the problem of estimating the treatment effect boils down to the problem of **estimating what would have happened to the outcome of unit $j = 1$ if it had not been treated**.

![img](https://matheusfacure.github.io/python-causality-handbook/_images/synth_img.png)

To estimate $Y_{1 t}^{N}$, we remember that a combination of units in the donor pool may approximate the characteristics of the treated unit much better than any untreated unit alone. Thus, a synthetic control is defined as a weighted average of the units in the control pool. Given the weights $W W = \left(\right. w_{2} , . . . , w_{J + 1} \left.\right)$, the synthetic control estimate of $Y_{1 t}^{N}$ is

$\hat{Y}_{1 t}^{N} = \sum_{j = 2}^{J + 1} w_{j} Y_{j t}$
...
<table><thead><tr><th></th><th>state</th><th>1</th><th>2</th><th>3</th><th>...</th><th>37</th><th>38</th><th>39</th></tr><tr><th></th><th>year</th><th></th><th></th><th></th><th></th><th></th><th></th><th></th></tr></thead><tbody><tr><th rowspan="5">cigsale</th><th>1970</th><td>89.800003</td><td>100.300003</td><td>123.000000</td><td>...</td><td>114.500000</td><td>106.400002</td><td>132.199997</td></tr><tr><th>1971</th><td>95.400002</td><td>104.099998</td><td>121.000000</td><td>...</td><td>111.500000</td><td>105.400002</td><td>131.699997</td></tr><tr><th>1972</th><td>101.099998</td><td>103.900002</td><td>123.500000</td><td>...</td><td>117.500000</td><td>108.800003</td><td>140.000000</td></tr><tr><th>1973</th><td>102.900002</td><td>108.000000</td><td>124.400002</td><td>...</td><td>116.599998</td><td>109.500000</td><td>141.199997</td></tr><tr><th>1974</th><td>108.199997</td><td>109.699997</td><td>126.699997</td><td>...</td><td>119.900002</td><td>111.800003</td><td>145.800003</td></tr></tbody></table>
...
![_images/15-Synthetic-Control_15_0.png](https://matheusfacure.github.io/python-causality-handbook/_images/15-Synthetic-Control_15_0.png)
...
![img](https://matheusfacure.github.io/python-causality-handbook/_images/out-of-sample.png)
...
$\hat{Y}_{j t}^{N} = \sum_{j = 2}^{J + 1} w_{j} Y_{j t}$

but now, we will use weights $W W = \left(\right. w_{2} , . . . , w_{J + 1} \left.\right)$ that minimises

$\left|\right. \left|\right. X X_{1} - X X_{0} W W \left|\right. \left|\right. = \left(\right. \sum_{h = 1}^{k} v_{h} \left(\right. X_{h 1} - \sum_{j = 2}^{J + 1} w_{j} X_{h j} \left.\right)^{2} \left.\right)^{\frac{1}{2}}$

subject to the restriction that $w_{2} , . . . , w_{J + 1}$ are positive and sum to one. Notice that $v_{h}$ reflect the importance of each variable when minimising the difference between the treated and the synthetic control. Different $v$ s would give different optimal weights. One way to choose $V$ is to make it so that each variable has mean zero and unit variance. A more complex way is to choose $V$ in such a way that variables that help to predict $Y$ better get higher importance. Since we want to keep the code simple, we will simply give the same importance for each variable.
...
![_images/15-Synthetic-Control_25_0.png](https://matheusfacure.github.io/python-causality-handbook/_images/15-Synthetic-Control_25_0.png)
...
![_images/15-Synthetic-Control_27_0.png](https://matheusfacure.github.io/python-causality-handbook/_images/15-Synthetic-Control_27_0.png)
...
|  | state | year | cigsale | after\_treatment | synthetic |
| --- | --- | --- | --- | --- | --- |
...
![_images/15-Synthetic-Control_36_0.png](https://matheusfacure.github.io/python-causality-handbook/_images/15-Synthetic-Control_36_0.png)
...
$M S E = \frac{1}{N} \sum \left(\right. Y_{t} - \hat{Y}_{t}^{S y n t h} \left.\right)^{2}$
...
![_images/15-Synthetic-Control_38_0.png](https://matheusfacure.github.io/python-causality-handbook/_images/15-Synthetic-Control_38_0.png)
...
$P V = \frac{1}{N} \sum 1 \left{\right. \hat{\tau}_{C a l i f} > \hat{\tau}_{j} \left.\right}$
...
![_images/15-Synthetic-Control_44_0.png](https://matheusfacure.github.io/python-causality-handbook/_images/15-Synthetic-Control_44_0.png)
...
- [[Synthetic Control]] — Synthetic control method with Python
