...
published: null
created: 2026-04-10
description: For mathematical and philosophical reasons, propensity scores and inverse
  probability weights don’t work in Bayesian inference. But never fear! There’s still
  a way to do it!
tags:
- clippings
doc_type: tutorial
...
![](https://www.andrewheiss.com/blog/2021/12/18/bayesian-propensity-scores-weights/index_files/figure-html/basic-dag-1.png)
...
![](https://www.andrewheiss.com/blog/2021/12/18/bayesian-propensity-scores-weights/index_files/figure-html/plot-propensity-hist-1.png)
...
![](https://www.andrewheiss.com/blog/2021/12/18/bayesian-propensity-scores-weights/index_files/figure-html/plot-iptw-pseudo-populations-1.png)
...
Here’s how Robins, Hernán, and Wasserman ([^2]), Zigler ([^5]), and Liao and Zigler ([^1]) explain it (but substantially simplified to the point of being a little bit wrong, but acceptably wrong). We can more formally define the average treatment effect (ATE) of mosquito nets on malaria risk using this estimand:

$$
\Delta_{\text{ATE}} = E \left[\right. \overset{\text{Average outcome } Y \text{when} \\ \text{treated},\text{ given confounders } X}{\overbrace{E \left(\right. Y_{i} \mid T_{i} = 1 , X_{i} \left.\right)}} - \overset{\text{Average outcome } Y \text{when} \\ \text{not treated},\text{ given confounders } X}{\overbrace{E \left(\right. Y_{i} \mid T_{i} = 0 , X_{i} \left.\right)}} \left]\right.
$$
...
$$
f \left(\right. \Delta \mid \mathbf{\mathit{T}} , \mathbf{\mathit{X}} , \mathbf{\mathit{Y}} \left.\right)
$$

This works well with Bayesian methods. We don’t know what $\Delta$ is, so we can use Bayes’ theorem to estimate it given our existing data for $T$, $X$, and $Y$. To quote from Liao and Zigler ([^1]),

> \[T\]raditional Bayeisan inference for $\Delta$ would follow from specification of a likelihood for $\left(\right. \mathbf{\mathit{T}} , \mathbf{\mathit{X}} , \mathbf{\mathit{Y}} \left.\right)$ conditional on unknown parameters, $\theta$, a prior distribution for $\theta$, and some function relating the data and $\theta$ to the quantity $\Delta$.
...
$$
\overset{\text{Posterior estimate} \\ \text{of } \Delta ,\text{ given data}}{\overbrace{P \left[\right. \Delta \mid \left(\right. \mathbf{\mathit{T}} , \mathbf{\mathit{X}} , \mathbf{\mathit{Y}} \left.\right) \left]\right.}} \propto \overset{\text{Likelihood for existing} \\ \text{data},\text{ given unknown } \Delta}{\overbrace{P \left[\right. \left(\right. \mathbf{\mathit{T}} , \mathbf{\mathit{X}} , \mathbf{\mathit{Y}} \left.\right) \mid \Delta \left]\right.}} \times \overset{\text{Prior} \\ \text{for } \Delta}{\overbrace{P \left[\right. \Delta \left]\right.}}
$$

But guess what’s missing entirely from this equation?! Our weights! Propensity scores and inverse probability weights have no place in this kind of Bayesian estimation. They would theoretically show up in the likelihood part of the Bayesian equation, but in practice, weights aren’t part of the data-generating process and thus aren’t actually part of the likelihood. This is the key problem with Bayesian weights: these weights **are not part of the model** for calculating the ATE ($f \left(\right. \Delta \mid \mathbf{\mathit{T}} , \mathbf{\mathit{X}} , \mathbf{\mathit{Y}} \left.\right)$). We can’t set a prior on the weight parameter—there isn’t even a weight parameter to work with in the likelihood. In order to be a true Bayesian, we can’t use propensity scores and weighting. This is the argument that Robins, Hernán, and Wasserman ([^2]) make: because propensity scores (and thus weights) aren’t part of the likelihood, we can’t do anything Bayesian with them.
...
Fortunately, Robins, Hernán, and Wasserman ([^2]) conclude by saying that there are possible compromises that we can use to work with weights in a Bayesian framework, and Liao and Zigler ([^1]) explore one of these compromises and propose a method of incorporating propensity scores into Bayesian estimation of causal effects. Their general approach is to think of the propensity score calculation as a new parameter $\nu$ (nu). Instead of calculating a single value of $\nu$ (i.e. a single set of propensity scores or weights) like we did with frequentist estimation, we incorporate a range of reasonable values of $\nu$ from the posterior distribution of the treatment/design model into the outcome model, which essentially lets us get rid of the $\nu$ term. Mathematically the approach looks like this:

$$
\overset{\text{Estimand for} \\ \text{the ATE},\text{ without } \nu}{\overbrace{f \left(\right. \Delta \mid \mathbf{\mathit{T}} , \mathbf{\mathit{X}} , \mathbf{\mathit{Y}} \left.\right)}} = \int_{\nu} \overset{\text{Outcome model} \\ \text{with } \nu}{\overbrace{f \left(\right. \Delta \mid \mathbf{\mathit{T}} , \mathbf{\mathit{X}} , \mathbf{\mathit{Y}} , \nu \left.\right)}} \overset{\text{Treatment model} \\ \text{creating propensity} \\ \text{scores with } T \text{and } X}{\overbrace{f \left(\right. \nu \mid \mathbf{\mathit{T}} , \mathbf{\mathit{X}} \left.\right)}} d \nu
$$

The $f \left(\right. \nu \mid \mathbf{\mathit{T}} , \mathbf{\mathit{X}} \left.\right)$ part of the equation is the treatment/design model and it doesn’t have any information about the outcome $Y$ in it. In our running example, this is `net ~ confounders`. The $f \left(\right. \Delta \mid \mathbf{\mathit{T}} , \mathbf{\mathit{X}} , \mathbf{\mathit{Y}} , \nu \left.\right)$ part of the equation is the outcome model and it incorporates $\nu$. This *isn’t* the final estimand, which doesn’t have the propensity scores ($\nu$) in it. We ultimately get rid of that $\nu$ term by marginalizing over the distribution for $\Delta$.
...
![](https://www.andrewheiss.com/blog/2021/12/18/bayesian-propensity-scores-weights/index_files/figure-html/visualize-ates-1.png)

We did it!

## Things I don’t know yet but want to know!

So, thanks to Liao and Zigler ([^1]), we have a fully Bayesian treatment/design model and we can incorporate the uncertainty from that model’s propensity scores and weights into the outcome/analysis model. That’s so neat!
...
Heiss, Andrew. 2021. “How to Use Bayesian Propensity Scores and Inverse Probability Weights.” December 18, 2021. [https://doi.org/10.59350/nrwsd-3jz20](https://doi.org/10.59350/nrwsd-3jz20).

[^1]: Liao, Shirley X., and Corwin M. Zigler. 2020. “Uncertainty in the Design Stage of Two-Stage Bayesian Propensity Score Analysis.” *Statistics in Medicine* 39 (17): 2265–90. [https://doi.org/10.1002/sim.8486](https://doi.org/10.1002/sim.8486)

[^2]: Robins, James M., Miguel A. Hernán, and Larry Wasserman. 2015. “On Bayesian Estimation of Marginal Structural Models.” *Biometrics* 71 (2): 296–99. [https://doi.org/10.1111/biom.12273](https://doi.org/10.1111/biom.12273)

[^3]: Rubin, Donald B. 1987. *Multiple Imputation for Nonresponse in Surveys*. New York: John Wiley & Sons. [https://doi.org/10.1002/9780470316696](https://doi.org/10.1002/9780470316696)
