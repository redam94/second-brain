...
> - Need the general Kalman filter / RTS smoother machinery (predict-update, smoothing, likelihood)? → [[Bayesian Statistics/Causal Inference/Time Series Causal Inference/State-Space and Kalman Filter/_Index|State-Space and Kalman Filter]]
...
|-----------|-------|--------|
| [[Bayesian Statistics/Causal Inference/Time Series Causal Inference/State-Space and Kalman Filter/_Index\|State-Space and Kalman Filter]] | 5 | Linear-Gaussian state-space models, the Kalman filter (predict-update recursion), the RTS smoother, and the marginal likelihood via the prediction-error decomposition — Särkkä (2013). The general machinery underlying BSTS. |
