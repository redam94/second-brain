...
**Covered by:** [[Markov Equivalence and CPDAGs]], [[PC Algorithm]], [[Conditional Independence Testing]], [[Greedy Equivalence Search]] (all in `Causal Discovery/`, created 2026-09-23)

**Previously partially addressed (2026-06-22):** The `Causal Discovery/` subfolder covered NOTEARS comprehensively: [[NOTEARS - Overview]], [[DAG Structure Learning Problem]], [[Smooth Characterization of Acyclicity]], [[NOTEARS Algorithm]], [[NOTEARS Experiments]]. The score-based continuous-optimization approach was well-documented. Constraint-based methods (PC algorithm, conditional independence testing) and score-based search (GES / Greedy Equivalence Search) were still absent — now added.
...
### 58. Systemic Risk Measures (MES, CoVaR, SRISK)
**Status:** 🌱 new

**Why it's a gap:**
[[Factor Copula Application - S&P 100 and Systemic Risk]] defines and applies the **Marginal Expected Shortfall** (MES) formula inline — $MES_{it} = -E_{t-1}[r_{it} \mid r_{mt} < C]$ — and introduces the multi-stock variant $kES$. The Brownlees & Engle (2011) SRISK framework is cited as a benchmark. But no note in the vault explains the systemic risk measurement literature: what MES, CoVaR (Adrian & Brunnermeier 2011), and SRISK *are*, why they differ, how they are estimated (non-parametric historical simulation, GARCH-based forecasting, factor copula methods), and what regulatory context motivates them (Basel III, SIFI identification, stress testing). The factor copula notes are the *application*, but the measurement framework they serve is undocumented. This leaves the vault's highest-dimensional application (100 S&P stocks) floating without the conceptual anchor of *why systemic risk matters* and how these measures compare. The gap connects [[Factor Copula Application - S&P 100 and Systemic Risk]], gap #29 (GARCH), and gap #31 (EVT).

**Adjacent notes:** [[Factor Copula Application - S&P 100 and Systemic Risk]], [[Factor Copulas - Overview]], [[Tail Dependence in Factor Copulas]], [[SMM Estimation of Factor Copulas]], [[Dependence Measures for Copulas]]

**Suggested sources / search terms:**
- Brownlees & Engle (2017) — "SRISK: A Conditional Capital Shortfall Measure of Systemic Risk" (*Review of Financial Studies*)
...
- Engle, Jondeau & Rockinger (2015) — "Systemic Risk in Europe" (*Review of Finance*)
- Search: "MES marginal expected shortfall", "CoVaR systemic risk", "SRISK capital shortfall", "systemic risk measures comparison", "NYU Stern V-Lab systemic risk"

---

### 59. Multi-Touch Attribution (MTA) and Marketing Attribution Modeling
**Status:** 🌱 new

**Why it's a gap:**
[[Bayesian Media Mix Modeling - Overview]] operates at the aggregate (weekly/monthly) level — it estimates the total contribution of each media channel but cannot attribute individual conversions to specific touchpoints in a customer journey. [[Activity Bias in Advertising]] and [[Observational vs Experimental Methods in Advertising]] explain why passive attribution (last-click, first-click, linear rules) is biased. [[ROAS, mROAS, and Optimal Media Mix]] optimizes the marketing budget using posterior ROAS. But **multi-touch attribution (MTA)** — models that distribute conversion credit across the ordered sequence of ad impressions/clicks in an individual's journey — is entirely absent. MTA methods include: rule-based (uniform, time-decay, U-shaped), data-driven Shapley value approaches (cooperative game theory applied to channel combinations), Markov chain transition models (Anderl et al. 2016), and Bayesian survival-based approaches (Dalessandro et al. 2012). The key distinction from MMM: MTA is person-level and channel-sequence-aware, while MMM is population-level and time-aggregated; increasingly, practitioners use both and triangulate. The vault's marketing analytics stack (MMM + activity-bias awareness + ROAS optimization) is incomplete without at least one note on the individual-level attribution counterpart.

**Adjacent notes:** [[Bayesian Media Mix Modeling - Overview]], [[Activity Bias in Advertising]], [[Observational vs Experimental Methods in Advertising]], [[ROAS, mROAS, and Optimal Media Mix]], [[Advertising and Promotion Effects]]

**Suggested sources / search terms:**
- Dalessandro et al. (2012) — "Causally motivated attribution for online advertising" (*KDD*)
- Anderl et al. (2016) — "Mapping the customer journey: Lessons learned from graph-based online attribution modeling" (*Journal of Business Research*)
- Shapley (1953) — "A value for n-person games" — Shapley value foundation
- Li & Kannan (2014) — "Attributing conversions in a multichannel online marketing environment" (*Journal of Marketing Research*)
- Search: "multi-touch attribution marketing", "Shapley value marketing attribution", "Markov chain attribution model", "data-driven attribution Google", "MTA vs MMM marketing mix"
...
| Causal Structure Learning — PC/GES (#9) | [[Markov Equivalence and CPDAGs]], [[PC Algorithm]], [[Conditional Independence Testing]], [[Greedy Equivalence Search]] | 2026-09-23 |
...
| 2026-09-21 | Run 24: reviewed 10 notes (The Selection Problem, Design of Static Response Models, Copula Estimation, Synthetic Control, Frequentist Causal Estimation, Code Prompts for Causal Structure, Yoneda Lemma, Factor Copula Application - S&P 100 and Systemic Risk, Approximation Methods, Optimal Marketing Decisions and Forecasting). No existing gaps newly covered this run. Formatting fixes: corrected broken wikilink `[[Bayesian moderation analysis]]` → `[[Moderation Analysis]]` in Design of Static Response Models. Cross-links added: Approximation Methods — expanded See Also with `[[HMC and Stan in Practice]]`, `[[Introduction to Bayesian Computation]]`, `[[Bayesian Workflow - Overview]]`, `[[Hilbert Space Gaussian Processes]]`; Optimal Marketing Decisions and Forecasting — added new `## See Also` section linking `[[ROAS, mROAS, and Optimal Media Mix]]`, `[[Bayesian Estimation and Priors for MMM]]`, `[[MMM Model Selection and Application]]`, `[[Activity Bias in Advertising]]`, `[[Bayesian Workflow - Overview]]`; Factor Copula Application — added `[[Bayesian Copula Estimation]]` and `[[Dependence Measures for Copulas]]` to See Also; Copula Estimation — added `[[Factor Copulas - Overview]]` and `[[Factor Copula Application - S&P 100 and Systemic Risk]]` to See Also (cross-linking bivariate Bayesian copula to high-dimensional factor copula cluster); Code Prompts for Causal Structure — added `[[DAG Structure Learning Problem]]` and `[[NOTEARS - Overview]]` to See Also (data-driven vs. LLM-based causal structure elicitation); Yoneda Lemma — added `[[Adjoint Functors]]` and `[[Units and Counits]]` to See Also (Yoneda bijection ↔ adjunction unit correspondence). Added gaps #58 (Systemic Risk Measures: MES, CoVaR, SRISK) and #59 (Multi-Touch Attribution). Gaps #29 (GARCH), #31 (EVT), #17 (LKJ Distribution), #56 (INLA) reinforced by this run's notes. |
| 2026-09-23 | Gap #9 (Causal Structure Learning — PC algorithm and GES) fully covered. Searched arXiv, JMLR, semanticscholar.org, nature.com for Spirtes et al. (2000) "CPS", Chickering (2002) JMLR (GES), Colombo & Maathuis (2014) JMLR (stable PC) — all freely available at source but blocked by session egress proxy (jmlr.org, arxiv.org, semanticscholar.org all 403). Downloaded Chickering (2015) "Selective GES" from microsoft.com/research (saved as `raw/chickering2015-selective-GES.pdf`; 6-page PDF confirmed real); PDF text unextractable without poppler-utils. Created source marker notes `raw/spirtes2000-CPS-and-PC-algorithm.md` and `raw/chickering2002-GES-JMLR.md` documenting paper locations and download status. Created 4 notes in `Causal Discovery/`: [[Markov Equivalence and CPDAGs]] (Verma–Pearl characterisation, CPDAG def., Meek R1–R4, identifiability limit, 3-node example), [[PC Algorithm]] (faithfulness + causal sufficiency assumptions, skeleton learning algorithm, v-structure orientation, Meek propagation, PC-stable vs original, finite-sample behaviour), [[Conditional Independence Testing]] (partial correlation def., Fisher's Z test with $T$ statistic, conditional G-test, KCIT, multiple testing, BIC-equivalent $\alpha$, sep-sets for v-structure orientation), [[Greedy Equivalence Search]] (decomposable score def., BIC score, FES algorithm, BES algorithm, Meek Conjecture theorem, asymptotic consistency theorem, PC vs GES comparison table, FGES, 3-variable example). Updated `Causal Discovery/_Index.md` (5→9 notes, full routing table and concept map). |
