...
| 2026-08-30 | Run 21 (scheduled repair): batch frontmatter and tag audit run. No individual notes deepened this run — focus was vault-wide structural repairs. Frontmatter fixes: added `source/ingested` tag to 24 Market Response Models notes (all previously used only `source/hanssens-parsons-schultz-2001` without the standard pipeline flag); changed `type/book-overview` → `type/overview` in Statistical Rethinking - Overview and Mostly Harmless Econometrics - Overview (non-standard tag unified to taxonomy); added `doc_type: overview`, `source_location`, `folder`, `depends_on`, `used_by` to BDA3 - Overview; added `doc_type: overview`, `source_location`, `folder`, `depends_on`, `used_by` to Statistical Rethinking - Overview; added `doc_type: qa` and `folder: "Questions-and-Answers"` to all 5 Q&A notes (Common Pitfalls, Frequentist vs Bayesian, Multiple Comparisons, Causal Estimates, SMM for ABMs). Wikilink fixes: `[[Bayesian Propensity Scores and IPW]]` → `[[Bayesian Inverse Probability Weighting]]` in 7 remaining locations (Propensity Score in Bayesian CI, Bayesian CI _Index.md, Bayesian Outcome Models, General Structure of Bayesian CI, Causal Inference _Index.md, How to use Bayesian propensity scores Clipping, Q - Uncovering Causal Estimates); Dream/_Index.md gap #1 adjacent notes link also fixed. Remaining work for next run: Clippings folder (20 notes, 7 missing frontmatter fields each); `q- and a- learning.pdf` orphan in raw/ (no notes reference it); genuine missing concept stubs ([[Horseshoe and Regularized Horseshoe Priors]], [[LKJ distribution]], [[Conditional Expectation Function]]). |
