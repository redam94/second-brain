...
**Covered by:** [[PC Algorithm]], [[GES Algorithm]] (both in `Research/Causal Discovery/`, created 2026-09-16). The `Causal Discovery/_Index.md` updated to reflect all three paradigms. Sources: Spirtes, Glymour & Scheines (2000) for PC; Chickering (2002) for GES. Note: PDFs are freely available (MIT Press / JMLR open access) but could not be downloaded in this session due to network egress policy — notes written from training knowledge and WebSearch summaries.
...
| Causal Structure Learning — PC algorithm + GES (#9) | [[PC Algorithm]], [[GES Algorithm]] (in `Research/Causal Discovery/`) | 2026-09-16 |
...
| 2026-09-16 | Run 24 (scheduled ingest): Gap #9 (Causal Structure Learning — PC algorithm + GES) marked 🍂 covered. Searched for freely available sources: Spirtes, Glymour & Scheines (2000) *Causation, Prediction, and Search* (PC algorithm) and Chickering (2002) "Optimal Structure Identification with Greedy Search" (*JMLR* 3:507–554) are both openly licensed; network egress policy blocked jmlr.org and arxiv.org so PDFs could not be downloaded this session — notes written from training knowledge and WebSearch summaries. Gap #8 (vine copulas) attempted first (lower-numbered 🌿 gap) but all identified free sources (mistis.inrialpes.fr, arxiv.org) were blocked. Fell back to Gap #9. Created: [[PC Algorithm]] (covers faithfulness + Markov + sufficiency assumptions, skeleton algorithm with order-ℓ CI test expansion, v-structure SepSet rule, four Meek rules R1–R4, stable PC, CI test comparison table Fisher Z/G²/KCI, consistency theorem, 3-variable worked example) and [[GES Algorithm]] (decomposable score definition, BIC formula, FES Insert operator, BES Delete operator, Hauser–Bühlmann turning phase, Chickering consistency theorem, Markov equivalence non-identifiability discussion, PC/GES/NOTEARS comparison table, 3-variable worked example). Updated `Research/Causal Discovery/_Index.md`: concept_count 5→7, routing summary extended to all three paradigms, concept map expanded, cross-cutting concepts section extended. |
