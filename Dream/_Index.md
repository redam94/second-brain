...
| 2026-09-09 | Run 23: frontmatter sweep — 15 notes repaired, 6 broken wikilinks fixed. No existing gaps newly covered this run. Frontmatter fixes: added `folder: "Bayesian Statistics"`, `source_location`, `doc_type: textbook`, `depends_on: []`, `used_by` (8 notes) to Statistical Rethinking - Overview; fixed `type/book-overview` → `type/overview` tag in same note; added `source_location`, `doc_type: textbook`, `depends_on: []`, `used_by` (18 notes) to BDA3 - Overview; added `folder: "Category Theory"`, `depends_on: []`, `used_by` (21 notes) to Basic Category Theory - Overview; added `folder` to 9 Category Theory notes missing it: Functors (`Category Theory/Foundations`), Categories (`Category Theory/Foundations`), Adjoint Functors (`Category Theory/Adjunctions`), General Limits (`Category Theory/Limits and Colimits`), Pullbacks (`Category Theory/Limits and Colimits`), Universal Properties - Introduction (`Category Theory/Universal Properties`), Adjoints and Limits (`Category Theory/Synthesis`), Limits via Representables (`Category Theory/Synthesis`), Representable Functors (`Category Theory/Representables`); added `folder: "Bayesian Statistics/Regression Models"` and `source: "[[raw/Missing Data]]"` to Missing Data - Statistical Rethinking; added `folder: "Bayesian Statistics/Advanced Models"` and `source: "[[raw/Confirmatory Factor Analysis and Structural Equation Models in Psychometrics]]"` to Confirmatory Factor Analysis and SEM; added `folder: "Bayesian Statistics/Advanced Models"` and `source: "[[raw/Social Networks]]"` to Social Network Models. Broken wikilink fixes: replaced `[[Bayesian Propensity Scores and IPW]]` → `[[Bayesian Inverse Probability Weighting]]` + `[[Bayesian Propensity Score Weighting]]` in 6 files: Propensity Score in Bayesian CI, Bayesian Outcome Models, General Structure of Bayesian CI, Causal Inference/_Index, Clippings/How to use Bayesian propensity scores, Q - Uncovering Causal Estimates. Remaining gaps: 129 notes still missing `folder`; 100 missing `used_by`; 98 missing `depends_on`/`source_location`/`doc_type`/`aliases` — next run should prioritize Workflow and Model Assessment subfolder notes. |
